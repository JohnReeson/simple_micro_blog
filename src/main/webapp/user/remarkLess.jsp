...
<body>
	<div class='form-group'>
		<form method='post'>
			...
			<input type='button' onclick='remarkCreate(${microblog.id})' class='btn btn-info remark_button' value='评论' />
		</form>
	</div>
	<div class='list-group rf_content'>
		<c:forEach items="${remarks}" var="r" varStatus="status" begin="0"
			...
			<div class='list-group-item'>
				...
				&nbsp;&nbsp;|&nbsp;&nbsp;<a href='javascript:remarkDelete(${r.id},${r.microblogid})'>删除</a>
				</c:if>
				</div>
			</div>
			...
			<div class='well collapse'  id='reply${r.id}'>
				<form method='post'>
					...
					<input type='button' onclick='remarkReply(${r.id},${r.user.id},${microblog.id })' class='btn btn-info remark_button' value='回复' />
				</form>
			</div>
		</c:forEach>
	</div>
